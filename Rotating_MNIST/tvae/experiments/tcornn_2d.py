...
from tvae.models.mlp import LinearFC_Decoder, LinearFC_Encoder
...
def create_model(n_caps, cap_shape, group_kernel=(5,5,1), mu_init=5.0, padding_mode='circular', 
    ...
    encoder = Gaussian_Encoder(model=LinearFC_Encoder(in_features=x_dim, out_features=z_dim))

    decoder = Bernoulli_Decoder(model=LinearFC_Decoder(in_features=h_dim, out_features=x_dim))
    ...
    rnn = Det_CoRNN_Caps(input_size=x_dim, n_caps=n_caps, cap_shape=cap_shape, dt=dt, gamma=gamma, alpha=alpha,
                ...
                driving=Conv_Cap2Cap(n_caps=n_caps, cap_shape=cap_shape, kernel_size=kernel_shape, stride=stride, 
                                    padding=padding, external_padding=external_padding, mult_init=1.0, share_over_caps=share_over_caps,
                                    forward_only=forward_only, locally_connected=locally_connected),
                padder=lambda x: F.pad(x, (external_padding, external_padding, external_padding, external_padding), mode=padding_mode),
                n_internal_steps=n_internal_steps, layernorm=False)
...
def main():
    ...
    name = 'TCoRNN-2D_Rot-MNIST_noLNrnn_wLNgrouper_posVel'
    ...
    for e in range(config['max_epochs']):
        ...
        (total_loss, total_neg_logpx_z, total_kl, num_batches) = train_epoch_autoregressive(model, optimizer, 
                                                    train_loader, log,
        ...
        if e % config['eval_epochs'] == 0:
            total_loss, total_neg_logpx_z, total_kl, total_is_estimate, num_batches = eval_epoch_autoregressive(model, test_loader, log, savepath, e, 
                                                                                                                ...
                                                                                                                wandb_on=config['wandb_on'],
                                                                                                                plot_waves=True, eval_batches=10)
