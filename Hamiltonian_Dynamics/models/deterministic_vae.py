...
class DeterministicLatentsGenerativeModel(base.SequenceModel[_ArrayOrPhase]):
  ...
  def __init__(
    ...
    elif self.latent_dynamics_type == "Waves":
      model_constructor = functools.partial(
          dynamics.WaveDynamicsNetwork,
          system_dim=self.latent_system_dim,
          name="Waves",
          **latent_system_kwargs
      )
    elif self.latent_dynamics_type == "NS":
      model_constructor = functools.partial(
          dynamics.NSDynamicsNetwork,
          system_dim=self.latent_system_dim,
          name="NavierStokes",
          **latent_system_kwargs
      )
    elif self.latent_dynamics_type == "CO":
      model_constructor = functools.partial(
          dynamics.CoupledOscillatorNetwork,
          system_dim=self.latent_system_dim,
  ...
  def process_latents_for_dynamics(self, z: jnp.ndarray) -> _ArrayOrPhase:
    ...
    # if self.latent_dynamics_type == "Waves":
    #   # Upsample low-dim output
    #   B, CHW = z.shape
    #   HW = np.sqrt(CHW // 2.0).astype(int)
    #   upsize = 69
    #   z_img = z.reshape(B, 2, HW, HW)
    #   z_up = jmg.resize(z_img, shape=(B, 2, upsize, upsize), method='nearest')
    #   return z_up.reshape(B, 2*upsize*upsize)
    return z
  ...
  def process_latents_for_decoder(self, z: _ArrayOrPhase) -> jnp.ndarray:
    ...
    # if self.latent_dynamics_type == "Waves":
    #   # Downsample high-dim dynamics
    #   B, T, CHW = z.shape
    #   HW = np.sqrt(CHW // 2.0).astype(int)
    #   z_img = z.reshape(B, T, 2, HW, HW)
    #   z_up = jmg.resize(z_img, shape=(B, T, 2, 23, 23), method='nearest')
    #   return z_up.reshape(B, T, 2*23*23)
    if self.latent_dynamics_type == "NSGN":
      if self.render_from_q_only:
        # Only return density (last third)
        z_third = z.shape[2] // 3
        return z[:, :, 2*z_third:]
      else:
        return z
    if self.latent_dynamics_type == "CO":
      if self.render_from_q_only:
        # z_half = z.shape[2] // 2
        z_pos = z[:,:,::2]
        
        # z_pos_anal = utils.hilbert_transform(z_pos, axis=1)
        # z_pos = jnp.abs(z_pos_anal)
  ...
  def reconstruct(
    ...
    p_x, _, _, _, z, _ = self._models_core(
        params=params,
        keys=jnr.split(rng, 6),
        image_data=image_data,
        use_mean=use_mean,
    ...
    return p_x, z # jnp.expand_dims(jnp.expand_dims(z[0,0], 0), 0)
