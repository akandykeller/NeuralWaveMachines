...
# SnapshotNT = {"id": None, "pickle_nest": None}
# CheckpointNT = {"active": None, "history": None}
...
class FileCheckpointer:
  """A Checkpointer reliant on an in-memory global dictionary, but also saves to disk."""
  ...
  def __init__(self, config, mode: str):
    ...
    if config.logger_backend == 'wandb':
      import wandb
      if config.reload_path is None:
        self.checkpoint_dir = wandb.run.dir
      else:
        self.checkpoint_dir = config.reload_path
    else:
      assert config.reload_path is None
      self.checkpoint_dir = config.checkpoint_dir
    
    del mode

  def fname(self, name):
    return os.path.join(self.checkpoint_dir, name + '.pkl')
  ...
  def _override_or_insert_from_pkl(self, current_state, snapshot):
    """Update the current state based on a snapshot."""
    for sk, sv in snapshot.items():
      if sk == 'experiment_ckpt':
        for kk in sv:
          setattr(current_state['experiment_module'], kk, sv[kk])
      elif sk == 'train_step_rng':
        # import ipdb; ipdb.set_trace()
        current_state[sk] = jax.device_put_replicated(sv[0], jax.devices()) # jax.device_put(sv)
      else:
        current_state[sk] = sv
  ...
  def save(self, ckpt_series: str) -> None:
    ...
    if len(series.history) > self._max_checkpoints_to_keep:
      GLOBAL_CHECKPOINT_DICT[ckpt_series] = series._replace(
          history=series.history[-self._max_checkpoints_to_keep:])
    logging.info("Saved checkpoint %s with id %s in memory.", ckpt_series, id_)
    with open(self.fname(ckpt_series), 'wb') as f :
      experiment = series.history[-1].pickle_nest.experiment_module
      global_step = series.history[-1].pickle_nest.global_step
      train_step_rng = series.history[-1].pickle_nest.train_step_rng
      experiment_ckpt = {}
      for kk in experiment.CHECKPOINT_ATTRS:
        experiment_ckpt[kk] = getattr(experiment, kk)
      for kk in experiment.NON_BROADCAST_CHECKPOINT_ATTRS:
        experiment_ckpt[kk] = getattr(experiment, kk)

      dumpable = {"experiment_ckpt": experiment_ckpt, "global_step": global_step, 'train_step_rng': train_step_rng}
      pickle.dump(dumpable, f)
    logging.info("Saved checkpoint %s to file %s", ckpt_series, self.fname(ckpt_series))

  def can_be_restored_from_memory(self, ckpt_series: str) -> bool:
    """Returns whether or not a checkpoint series is present in-memory"""
    return ((ckpt_series in GLOBAL_CHECKPOINT_DICT) and
              GLOBAL_CHECKPOINT_DICT[ckpt_series].history)

  def can_be_restored(self, ckpt_series: str) -> bool:
    """Returns whether or not a given checkpoint series can be restored."""
    return (self.can_be_restored_from_memory(ckpt_series)
            or ckpt_series + '.pkl' in os.listdir(self.checkpoint_dir))

  def restore(self, ckpt_series: str) -> None:
    """Restores the checkpoint."""
    if self.can_be_restored_from_memory(ckpt_series):
      snapshot = GLOBAL_CHECKPOINT_DICT[ckpt_series].history[-1].pickle_nest
      current_state = self.get_experiment_state(ckpt_series)
      self._override_or_insert(current_state, snapshot)
    else:
      with open(self.fname(ckpt_series), 'rb') as f:
        dumpable = pickle.load(f)
        current_state = self.get_experiment_state(ckpt_series)
        self._override_or_insert_from_pkl(current_state, dumpable)
        id_ = 0
        snapshot = copy.copy(current_state)
        GLOBAL_CHECKPOINT_DICT[ckpt_series].history.append(SnapshotNT(id_, snapshot))
    logging.info("Returned checkpoint %s with id %s.", ckpt_series,
                 GLOBAL_CHECKPOINT_DICT[ckpt_series].history[-1].id)
